...
import shop.models
from cart.models import User, Cart, CustomUser
from cart.permissions import IsOwnerOrReadOnly
from cart.serializers import CartSerializer, LoginUser, RegisterUser, UserAdminSerializer
...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def cart_to_speech(self, request, pk):
        cart = Cart.objects.filter(user=self.request.user).values_list('products')
        products = []
        for c in cart:
            for c1 in c:
                product = str(shop.models.Product.objects.get(pk=c1))
